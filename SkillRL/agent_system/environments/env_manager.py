...
def make_envs(config):
    ...
    if "xskill" in config.env.env_name.lower():
        xskill_cfg = config.env.get("xskill", {})
        repo_root = xskill_cfg.get("repo_root", None) or os.environ.get("XSKILL_REPO_ROOT")
        if not repo_root:
            raise ValueError("XSkill environment requires env.xskill.repo_root or XSKILL_REPO_ROOT.")
        if repo_root not in sys.path:
            sys.path.insert(0, repo_root)
        from xskill_rl.skillrl_bridge import make_xskill_envs_from_skillrl

        return make_xskill_envs_from_skillrl(config)
