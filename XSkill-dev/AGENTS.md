...
当前仓库新增了 SkillRL 风格 GRPO 适配层，但仍不内置完整分布式 trainer：

- `xskill_rl/skillrl/skill_bank.py`: SkillRL `SkillBank` JSON、`SkillsOnlyMemory` template/embedding 检索与 prompt 格式化。
- `xskill_rl/skillrl/grpo.py`: GRPO group advantage 与默认超参数记录。
- `xskill_rl/skillrl/verl_export.py`: 将 XSkill 标准样本导出为 SkillRL/verl `RLHFDataset` 风格记录。
- `xskill_rl/skillrl/config.py`: 生成对齐 SkillRL 官方脚本的 Hydra overrides。
- `xskill_rl/skillrl/dynamic_update.py`: 失败 trajectory 选择与动态 skill update prompt 辅助。
- `scripts/convert_xskill_skill_to_skillrl_bank.py`: 将 XSkill `SKILL.md` 转成 SkillRL SkillBank JSON。
- `scripts/prepare_skillrl_grpo_dataset.py`: 导出 SkillRL/verl GRPO JSONL 或 Parquet。
- `scripts/build_skillrl_grpo_command.py`: 生成 SkillRL 风格 `python -m verl.trainer.main_ppo ...` 命令。

注意：这些改动只接入 SkillRL 的数据、SkillBank、GRPO 配置和外部训练入口，不修改 `eval/` 的 XSkill inference/memory 主链路。完整 online rollout、参数更新和分布式训练仍应在外部 SkillRL/verl 环境实现或注册 XSkill visual reasoning environment 后运行。
